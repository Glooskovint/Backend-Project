...
export default function Presupuesto({
  ...
  onClose,
  isExportMode = false,
  containerId = "presupuesto-chart-render-area-pdf",
}) {
  ...
  useEffect(() => {
    ...
    const calcularTotal = (tasksList) => {
      return tasksList.reduce((acc, task) => {
        ...
        return acc + totalTarea; // no suma subtareas
      }, 0);
    };
  ...
  const renderTarea = (task, level = 0, path = []) => {
    ...
    const tieneSubtareas =
      task.subtareas &&
      Array.isArray(task.subtareas) &&
      task.subtareas.length > 0;
    const paddingLeft = `12px`; // Indent subtasks

    return (
      <React.Fragment key={task.id}>
        <tr className="hover:bg-gray-50 dark:hover:bg-gray-700">
          <td className="px-4 py-2 border dark:border-gray-600 text-rigth align-top">
            {taskNumber}
          </td>
          <td
            style={{ paddingLeft }}
            className="px-4 py-2 border dark:border-gray-600 font-medium align-top text-text-main dark:text-text-main-dark"
          >
            {task.nombre}
          </td>
  ...
  const presupuestoTableContent = (
    <div
      className={`presupuesto-table-container ${
        isExportMode ? "bg-white p-4" : "overflow-x-auto"
      }`}
      ...
      <table className="min-w-full border border-gray-200 dark:border-gray-600 text-sm">
        <thead className="bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200">
          <tr>
            ...
            <th className="px-4 py-2 border dark:border-gray-600 text-left">
              Nombre de Tarea
            </th>
            <th className="px-4 py-2 border dark:border-gray-600 text-right w-40">
              Presupuesto (S/)
            </th>
          </tr>
        </thead>
        <tbody className="text-text-main dark:text-text-main-dark">
          ...
          <tr className="bg-gray-100 dark:bg-gray-700 font-semibold text-gray-800 dark:text-gray-100">
            <td
              colSpan={2}
              className="px-4 py-2 border dark:border-gray-600 text-right"
            >
              Total Proyecto:
            </td>

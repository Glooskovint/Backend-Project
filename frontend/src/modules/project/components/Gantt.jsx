...
export default function Gantt({ projectId, onClose, isExportMode = false, containerId = "gantt-chart-render-area-pdf", exportMaxDepth = null }) {
  ...
  // This is the actual HTML structure for the Gantt chart content
  const ganttChartDisplayContent = (
    <div
      className={`gantt-chart-render-container ${isExportMode ? 'bg-white p-4' : 'flex-grow overflow-auto'}`}
      // Apply the containerId when in export mode
      id={isExportMode ? containerId : undefined}
      style={isExportMode ? { width: `${TASK_LEGEND_WIDTH + (totalTimelineWidth || 800) + 40}px`, overflow: 'visible' } : {}}
    >
      <div className="grid" style={{ gridTemplateColumns: `${TASK_LEGEND_WIDTH}px 1fr` }}>
        {/* Leyenda izquierda */}
        <div className={isExportMode ? "bg-white z-20" : "sticky left-0 bg-white z-20"}>
          ...
          <ul className="list-none m-0 p-0">
            {allRows.map(row => (
              <li
                ...
                style={{ height: `${ROW_HEIGHT}px`, paddingLeft: `${row.level * 10 + 12}px` }}
              >
                {row.hasSubtasks && (
                  <button
                    onClick={() => toggleExpand(row.id)}
                    className="mr-1 text-xs text-gray-600 hover:text-black"
                    // Disable button in export mode as interaction is not needed
        ...
        <div className="relative">
          <div className={isExportMode ? "bg-white z-10" : "sticky top-0 bg-white z-10"}>
            {/* Meses */}
